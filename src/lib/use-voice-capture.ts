...
type Capture = { stream: MediaStream; context: AudioContext; source: MediaStreamAudioSourceNode; processor: ScriptProcessorNode };
 ...
 */
export function useVoiceCapture({ onText, onSpeechStart, onError }: Options) {
  const [listening, setListening] = useState(false);
  const captureRef = useRef<Capture | null>(null);
  const handlers = useRef({ onText, onSpeechStart, onError });
  useEffect(() => { handlers.current = { onText, onSpeechStart, onError }; });
  ...
  const transcribe = useCallback(async (audio: Blob) => {
    ...
    try {
      const response = await fetch(`/api/transcribe?lang=${lang}`, { method: "POST", headers: { "Content-Type": "audio/wav" }, body: audio });
  ...
  const start = useCallback(async () => {
    ...
    processor.onaudioprocess = (event) => {
      ...
      const threshold = Math.max(MIN_LEVEL, noiseFloor * 2.5);
      ...
      if (level > threshold * 0.7) { silenceMs = 0; speechMs += frameMs; } else silenceMs += frameMs;
      if (silenceMs >= END_SILENCE_MS || frames.length * frameMs >= MAX_UTTERANCE_MS) {
        ...
        void transcribe(encodeWav(utterance, context.sampleRate));
      }
    };
